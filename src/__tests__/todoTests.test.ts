...
describe("the todo app", () => {
  ...
  await driver.findElement(tp.todoInput).sendKeys("Do Homework\n");
});
...
it("can remove a todo", async () => {
  ...
  let myTodo = await myTodos.filter(async (todo) => {
    (await (await todo.findElement(tp.todoLabel)).getText()) == "Do Homework";
  });
...
it("can mark a todo with a star", async () => {
  await driver.wait(until.elementLocated(tp.todoInput));
    ...
    await driver.findElement(tp.todoInput).sendKeys("Make Dinner\n");
